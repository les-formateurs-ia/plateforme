...
function buildScriptPrompt(lessonTitle: string, lessonContent: string, objectifProfessionnel: string | null, pedagogyStyle: string): string {
...
RÈGLE ABSOLUE : le contenu pédagogique obligatoire du cours (Source 1) doit être fidèlement transmis. Ne supprime jamais une connaissance ou un point du cours pour "faire de la place" à la personnalisation — la personnalisation est une couche ajoutée, jamais un remplacement. La contrainte de longueur ci-dessous (Format de sortie) est également non négociable, y compris face au style pédagogique ci-dessous.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const scriptPrompt = buildScriptPrompt(lesson.title, lesson.reference_content, objectifProfessionnel, pedagogyStyle);
