...
Deno.serve(async (req) => {
  ...
  try {
    const { lessonId, chunkCount, sampleRate, channels, transcript } = await req.json();
    ...
    for (let i = 0; i < chunkCount; i++) {
      const path = `${userId}/tmp/${lessonId}/${i}.pcm`;
      tmpPaths.push(path);
    ...
    const storagePath = `${userId}/${lessonId}.wav`;
    ...
    await supabase.from("ai_generated_content").delete()
      .eq("user_id", userId).eq("lesson_id", lessonId).eq("content_type", "podcast");
    const { error: insertErr } = await supabase.from("ai_generated_content").insert({
      user_id: userId,
      lesson_id: lessonId,
      content_type: "podcast",
      source_prompt: "Voir generate-podcast-script (script personnalisé du dialogue).",
    ...
    return jsonResponse({ status: "ok", storagePath });
  } catch (err) {
