// Duplicat indépendant pour le module "Exercez-vous 2" (tables *_2, bucket media-exercise-outputs-2) — voir supabase/migrations/0065_practice2_module.sql. Logique identique à check-media-exercise-status.
...
async function signedUrl(
  ...
  if (!path) return null;
  const { data } = await supabase.storage.from("media-exercise-outputs-2").createSignedUrl(path, 3600);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: row, error: rowErr } = await supabase
      .from("media_exercise_attempts_2")
      .select("*")
      .eq("id", attemptId)
      .maybeSingle();
    ...
    if (originalOp.error || correctedOp.error) {
      ...
      const { data: updated } = await supabase.from("media_exercise_attempts_2").update({ status: "failed", error: errMsg }).eq("id", attemptId).select().single();
    ...
    if (!originalVideo?.uri || !correctedVideo?.uri) {
      const { data: updated } = await supabase.from("media_exercise_attempts_2").update({
    ...
    const [up1, up2] = await Promise.all([
      supabase.storage.from("media-exercise-outputs-2").upload(originalPath, originalBytes, { contentType: "video/mp4", upsert: true }),
      supabase.storage.from("media-exercise-outputs-2").upload(correctedPath, correctedBytes, { contentType: "video/mp4", upsert: true }),
    ]);
    if (up1.error || up2.error) return jsonResponse({ error: up1.error?.message || up2.error?.message || "Échec de l'upload vidéo." }, 500);

    const { data: updated, error: updateErr } = await supabase.from("media_exercise_attempts_2").update({
