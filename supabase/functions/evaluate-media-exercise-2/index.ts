// Duplicat indépendant pour le module "Exercez-vous 2" (tables *_2, bucket media-exercise-outputs-2) — voir supabase/migrations/0065_practice2_module.sql. Logique identique à evaluate-media-exercise.
      ...
      try {
        if (mode === "image") {
          ...
          const [up1, up2] = await Promise.all([
            supabase.storage.from("media-exercise-outputs-2").upload(originalPath, originalBytes, { contentType: "image/png", upsert: true }),
            supabase.storage.from("media-exercise-outputs-2").upload(correctedPath, correctedBytes, { contentType: "image/png", upsert: true }),
          ]);
          if (up1.error || up2.error) throw new Error(up1.error?.message || up2.error?.message || "Échec de l'upload de l'image.");
          await supabase.from("media_exercise_attempts_2").update({
