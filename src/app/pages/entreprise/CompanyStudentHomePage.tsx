...
import { ClipboardList, FileText, Code2, Star, Upload, Download, CheckCircle2 } from "lucide-react";
...
export function CompanyStudentHomePage() {
  ...
  const load = async () => {
    ...
    try {
      const [company, pos, fls, exs, sat, cats, mine] = await Promise.all([
        ...
        listVisiblePositioningTests(companyId, user.id),
        listVisibleCompanyFiles(companyId),
        listVisibleCompanyHtmlExercises(companyId),
        listVisibleSatisfactionTests(companyId, user.id),
        listCompanyFileCategories(companyId),
        listMyCompanyUploads(companyId, user.id),
      ]);
  ...
  const handleUpload = async () => {
    if (!companyId || !user || !pendingFile || uploading) return;
    setUploading(true);
    try {
      const created = await uploadCompanyStudentFile(companyId, user.id, categoryId || null, pendingFile);
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 sm:px-6 lg:px-8 py-5 sm:py-6 space-y-8">
      <div>
        ...
        <p className="text-sm mt-0.5" style={{ color: th.fg3 }}>Ton espace de formation entreprise.</p>
      </div>
