...
export function EntrepriseChoicePage() {
  ...
  // Rideau temporaire : accès Entreprise coupé pour le formateur (le temps
  // de finaliser le module côté admin) — visuel seulement, à retirer une
  // fois le module ouvert aux formateurs.
  const entrepriseLocked = role === "formateur";

  const CHOICES = [
    { icon: GraduationCap, title: "E-learning CPF", desc: "Formations, leçons et suivi des élèves CPF.", path: `${staffBase}/courses`, locked: false },
    { icon: Building2, title: "Entreprise", desc: "Entreprises, collaborateurs, contenu et suivi B2B.", path: "/entreprise", locked: entrepriseLocked },
  ];
    ...
    <div className="relative min-h-screen flex items-center justify-center p-4" style={{ background: th.bg, fontFamily: "'Funnel Display',sans-serif" }}>
      ...
      <div className="relative z-10 w-full max-w-2xl fade-up">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {CHOICES.map(({ icon: Icon, title, desc, path, locked }) => (
            <div key={title} className="relative">
              <GCard glow={!locked} accent={!locked} onClick={locked ? undefined : () => navigate(path)} className={`p-6 flex flex-col items-start gap-3 transition-transform ${locked ? "grayscale opacity-40" : "hover:scale-[1.02]"}`}>
              ...
              {locked && (
                <div className="absolute inset-0 rounded-2xl cursor-not-allowed" style={{ background: "rgba(120,120,120,0.35)" }} title="Bientôt disponible" />
              )}
            </div>
          ))}
        </div>
