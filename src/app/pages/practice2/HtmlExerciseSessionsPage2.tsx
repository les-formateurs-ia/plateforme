...
import { HtmlExerciseEditDialog2 } from "@/app/components/practice2/HtmlExerciseEditDialog2";
import { TagManagerDialog2 } from "@/app/components/practice2/TagManagerDialog2";
import { ExerciseTagPill } from "@/app/components/practice/ExerciseTagPill";
import { listVisibleHtmlExercises, ensureHtmlExerciseSession, type VisibleHtmlExercise } from "@/app/lib/htmlExercise2";
import { listHtmlExercises, type HtmlExerciseRow } from "@/app/lib/htmlExercises2";
import type { ExerciseTag } from "@/app/lib/exerciseTags2";
...
function AdminHtmlExercisesView() {
  ...
  return (
    <>
      ...
      <HtmlExerciseEditDialog2 open={dialogOpen} onOpenChange={setDialogOpen} exercise={editingExercise} onSaved={load} />
      <TagManagerDialog2 open={tagManagerOpen} onOpenChange={setTagManagerOpen} onChanged={load} />
    </>
  );
}
...
export function HtmlExerciseSessionsPage2() {
  const th = useTh();
  const navigate = useNavigate();
  const { role } = useAuth();
