...
import { listPromptExerciseSessions, deletePromptExerciseSession, type PromptExerciseSession } from "@/app/lib/promptExercise";
...
function scoreTone(score: number): string {
  if (score >= 16) return "#6adeb1";
  if (score >= 10) return "#fbc2ad";
  return RED;
}
...
export function PromptSessionsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await listPromptExerciseSessions(user.id);
  ...
  const startNewSession = () => {
    ...
    navigate(`/practice/prompts/${sessionId}`);
  };
  ...
  const handleDelete = async (e: MouseEvent, sessionId: string) => {
    ...
    try {
      await deletePromptExerciseSession(user.id, sessionId);
    ...
    <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
      ...
      {!loading && !loadError && sessions.length > 0 && (
        <div className="space-y-3">
          ...
          {sessions.map((s) => (
            <GCard key={s.sessionId} className="hover:scale-[1.005] transition-transform" onClick={() => navigate(`/practice/prompts/${s.sessionId}`)}>
              <div className="p-5 flex items-center gap-4 cursor-pointer">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs font-bold px-2 py-0.5 rounded-full shrink-0" style={{ color: scoreTone(s.lastScore), background: `${scoreTone(s.lastScore)}1f` }}>{s.lastScore}/20</span>
                  </div>
