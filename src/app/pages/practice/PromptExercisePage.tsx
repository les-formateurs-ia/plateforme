...
import { useTh, type Th } from "@/app/theme/theme";
...
import {
  ...
  type PromptExerciseAttempt, type PromptCorrection,
} from "@/app/lib/promptExercise";

const RED = "#e5484d";
const GREEN = "#6adeb1";

interface LocatedCorrection extends PromptCorrection { index: number; start: number; end: number; anchored: boolean }
...
function locateCorrections(text: string, corrections: PromptCorrection[]): { anchored: LocatedCorrection[]; unanchored: LocatedCorrection[] } {
  const usedRanges: [number, number][] = [];
  const found: { start: number; end: number; correction: PromptCorrection }[] = [];
  const missed: PromptCorrection[] = [];
...
function renderAnnotatedText(text: string, anchored: LocatedCorrection[], th: Th): ReactNode[] {
  ...
  for (const c of anchored) {
    ...
    nodes.push(
      <span key={`c${c.index}`}>
        <span style={{ color: RED, textDecoration: "line-through", textDecorationThickness: "2px" }}>{c.excerpt}</span>
...
function scoreTone(score: number): { color: string; bg: string } {
  if (score >= 16) return { color: GREEN, bg: "rgba(106,222,177,0.12)" };
  if (score >= 10) return { color: "#fbc2ad", bg: "rgba(251,194,173,0.12)" };
  return { color: RED, bg: "rgba(229,72,77,0.12)" };
}
