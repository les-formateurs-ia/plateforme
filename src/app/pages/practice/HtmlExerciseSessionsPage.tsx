...
import { listHtmlExerciseSessions, deleteHtmlExerciseSession, type HtmlExerciseSession } from "@/app/lib/htmlExercise";
...
export function HtmlExerciseSessionsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const rows = await listHtmlExerciseSessions(user.id);
  ...
  const handleDelete = async (e: MouseEvent, sessionId: string) => {
    ...
    try {
      await deleteHtmlExerciseSession(user.id, sessionId);
    ...
    <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
      ...
      {!loading && !loadError && sessions.length > 0 && (
        <div className="space-y-3">
          ...
          {sessions.map((s) => (
            <GCard key={s.sessionId} className="hover:scale-[1.005] transition-transform" onClick={() => navigate(`/practice/html/${s.sessionId}`)}>
              <div className="p-5 flex items-center gap-4 cursor-pointer">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs truncate mt-0.5 font-mono" style={{ color: th.fg3 }}>{s.preview || "(vide)"}</p>
