...
export function StudioVideosPage() {
  ...
  const openDetail = async (gen: StudioVideoGeneration) => {
    ...
    if (gen.sourceImagePath) {
      try { setDetailSourceUrl(await getStudioVideoSignedUrl(gen.sourceImagePath)); } catch { /* ignore */ }
    }
  };
    ...
    <div className="flex-1 min-h-0 flex flex-col">
        ...
        <div className="rounded-3xl overflow-hidden max-w-[46%] mx-auto" style={{ background: th.card, border: `1px solid ${th.sep}`, boxShadow: "0 2px 10px rgba(0,0,0,0.18)", opacity: 0.8 }}>
          <div className="flex flex-wrap items-center gap-3 px-4 sm:px-5 py-3" style={{ borderBottom: `1px solid ${th.sep}` }}>
            <div className="flex items-center gap-2">
              ...
              <div className="w-[168px]"><VSelect sm value={modelId} onValueChange={changeModel} options={STUDIO_VIDEO_MODELS.map((m) => ({ value: m.id, label: m.label }))} disabled={generating} /></div>
            </div>
            {model.options.map((opt) => (
              <div key={opt.key} className="flex items-center gap-2">
                <span className="text-xs font-bold shrink-0" style={{ color: th.fg3 }}>{opt.label}</span>
                <Segmented
                  value={optionValues[opt.key] ?? opt.default}
                  onChange={(v) => setOptionValues((prev) => ({ ...prev, [opt.key]: v }))}
            ...
            {model.supportsSourceImage && (
              sourcePreview ? (
                <div className="relative w-16 h-16 rounded-2xl overflow-hidden shrink-0 group cursor-pointer" title="Voir la photo de référence" onClick={() => setSourcePreviewOpen(true)}>
      ...
      {sourcePreviewOpen && sourcePreview && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.7)" }} onClick={() => setSourcePreviewOpen(false)}>
          <div onClick={(e) => e.stopPropagation()} className="max-w-lg w-full rounded-2xl overflow-hidden" style={{ background: th.card, border: `1px solid ${th.sep}` }}>
            <img src={sourcePreview} alt="Photo de référence" className="w-full max-h-[70vh] object-contain" style={{ background: "#000" }} />
            <div className="p-5 space-y-2">
              <p className="text-sm" style={{ color: th.fg }}>Photo ajoutée comme référence (Image-to-Video).</p>
          ...
          <div onClick={(e) => e.stopPropagation()} className="max-w-lg w-full rounded-2xl overflow-hidden" style={{ background: th.card, border: `1px solid ${th.sep}` }}>
            ...
            <div className="p-5 space-y-2">
              <div className="flex items-start gap-3">
                {detailSourceUrl && (
                  <button onClick={() => setDetailSourceZoom(true)} title="Voir la photo de référence utilisée" className="relative w-11 h-11 rounded-xl overflow-hidden shrink-0 group">
                    <img src={detailSourceUrl} alt="Photo de référence" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: "rgba(0,0,0,0.35)" }}>
      ...
      {detailSourceZoom && detailSourceUrl && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.75)" }} onClick={() => setDetailSourceZoom(false)}>
          <div onClick={(e) => e.stopPropagation()} className="max-w-lg w-full rounded-2xl overflow-hidden" style={{ background: th.card, border: `1px solid ${th.sep}` }}>
            <img src={detailSourceUrl} alt="Photo de référence" className="w-full max-h-[70vh] object-contain" style={{ background: "#000" }} />
            <div className="p-5 space-y-2">
              <p className="text-sm" style={{ color: th.fg }}>Photo de référence utilisée pour cette génération.</p>
