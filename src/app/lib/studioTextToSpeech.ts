...
// via MiniMax Speech 2.8 sur l'API Runware. Catalogue de voix ET réglages
// expressifs (vitesse/volume/hauteur/émotion) dupliqués depuis
// supabase/functions/_shared/studio-tts-voices.ts (source de vérité, avec
// tout le contexte : origine du catalogue, bornes non documentées par
// Runware reprises de l'API MiniMax T2A, etc.) — à garder en phase, comme
// avant ce module (partagé aussi avec "Faites parler vos images").
...
export const TTS_VOICES: TtsVoice[] = TTS_LANGUAGES.flatMap((group) =>
  group.voices.map((v) => ({ id: v.id, label: `${group.label} — ${v.label}`, language: group.code })),
);
...
// "Automatique" n'est pas dans cette liste : c'est l'absence de sélection
// (undefined) qui laisse MiniMax choisir l'émotion la plus naturelle pour le
// texte — cf. studio-tts-voices.ts (backend) pour le détail.
...
export const TTS_SPEED_RANGE = { min: 0.5, max: 2, step: 0.1, default: 1 };
export const TTS_VOLUME_RANGE = { min: 0.1, max: 10, step: 0.1, default: 1 };
export const TTS_PITCH_RANGE = { min: -12, max: 12, step: 1, default: 0 };

export const SCRIPT_MAX_LENGTH = 4000;

export interface TtsRequestControls {
  speed?: number;
  volume?: number;
  pitch?: number;
  emotion?: string;
...
function mapRow(row: {
  ...
  speed: number | null; volume: number | null; pitch: number | null; emotion: string | null;
...
const TTS_SELECT = "id, status, script_text, voice, language, speed, volume, pitch, emotion, audio_path, error_message, created_at";
...
export async function requestTtsGeneration(params: { script: string; voice: string } & TtsRequestControls): Promise<string> {
