...
import { listExerciseSessions, deleteExerciseSession } from "@/app/lib/exerciseSessions2";
...
export async function listMediaExerciseSessions(userId: string): Promise<MediaExerciseSession[]> {
  const [sessions, attemptsResp] = await Promise.all([
    ...
    supabase.from("media_exercise_attempts_2").select("session_id, mode, score, created_at").eq("user_id", userId).order("created_at", { ascending: true }),
  ]);
...
export async function deleteMediaExerciseSession(userId: string, sessionId: string): Promise<void> {
  ...
  if (paths.length > 0) await supabase.storage.from("media-exercise-outputs-2").remove(paths);
...
export async function submitMediaExercise(promptText: string, sessionId: string, mode: MediaMode): Promise<MediaExerciseAttempt> {
  const { data, error } = await supabase.functions.invoke("evaluate-media-exercise-2", { body: { promptText, sessionId, mode } });
...
export async function pollMediaExerciseAttempt(
  ...
  while (Date.now() < deadline) {
    const { data, error } = await supabase.functions.invoke("check-media-exercise-status-2", { body: { attemptId } });
    if (error) throw new Error(await extractFunctionError(error));
    if (data?.error) throw new Error(data.error);
