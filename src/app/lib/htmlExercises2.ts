...
import type { ExerciseVisibility2 } from "@/app/lib/supabase/database.types";
import { listExerciseTagsForExercises, type ExerciseTag } from "@/app/lib/exerciseTags2";
...
export async function listHtmlExercises(visibility: ExerciseVisibility2): Promise<HtmlExerciseRow[]> {
  const { data: exercises, error } = await supabase
    .from("html_exercises_2")
  ...
  const { data: assignments, error: assignmentsError } = await supabase
    .from("html_exercise_assignments_2")
    .select("exercise_id")
...
export async function getExerciseAssignees(exerciseId: string): Promise<string[]> {
  const { data, error } = await supabase.from("html_exercise_assignments_2").select("student_id").eq("exercise_id", exerciseId);
  if (error) throw error;
...
export async function removeExerciseAssignee(exerciseId: string, studentId: string): Promise<void> {
  const { error } = await supabase.from("html_exercise_assignments_2").delete().eq("exercise_id", exerciseId).eq("student_id", studentId);
  if (error) throw error;
}
...
export async function updateHtmlExercise(id: string, patch: HtmlExerciseUpdate): Promise<void> {
  ...
  const { error: deleteError } = await supabase.from("html_exercise_assignments_2").delete().eq("exercise_id", id);
...
export async function deleteHtmlExercise(id: string): Promise<void> {
  const { error } = await supabase.from("html_exercises_2").delete().eq("id", id);
  if (error) throw error;
}

export async function updateHtmlExerciseContent(id: string, htmlContent: string): Promise<void> {
  const { error } = await supabase.from("html_exercises_2").update({ html_content: htmlContent }).eq("id", id);
  if (error) throw error;
}
