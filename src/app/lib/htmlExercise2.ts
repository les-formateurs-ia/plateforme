...
export async function getExerciseBriefForSession(sessionId: string): Promise<HtmlExerciseBrief | null> {
  const { data: session, error } = await supabase.from("exercise_sessions_2").select("exercise_id").eq("id", sessionId).maybeSingle();
  if (error) throw error;
