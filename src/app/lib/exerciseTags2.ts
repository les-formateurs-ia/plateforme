...
export async function listExerciseTags(): Promise<ExerciseTag[]> {
  const { data, error } = await supabase.from("exercise_tags_2").select("id, name").order("name", { ascending: true });
...
export async function renameExerciseTag(id: string, name: string): Promise<void> {
  const { error } = await supabase.from("exercise_tags_2").update({ name }).eq("id", id);
  if (error) throw error;
}

export async function deleteExerciseTag(id: string): Promise<void> {
  const { error } = await supabase.from("exercise_tags_2").delete().eq("id", id);
  if (error) throw error;
}
...
export async function getTagIdsForExercise(exerciseId: string): Promise<string[]> {
  const { data, error } = await supabase.from("html_exercise_tag_assignments_2").select("tag_id").eq("exercise_id", exerciseId);
  if (error) throw error;
...
export async function setExerciseTags(exerciseId: string, tagIds: string[], assignedBy: string): Promise<void> {
  const { error: deleteError } = await supabase.from("html_exercise_tag_assignments_2").delete().eq("exercise_id", exerciseId);
  if (deleteError) throw deleteError;
  if (tagIds.length) {
    const { error: insertError } = await supabase
      .from("html_exercise_tag_assignments_2")
